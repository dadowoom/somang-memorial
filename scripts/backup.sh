...
for cmd in node mysqldump tar aws; do
  command -v "$cmd" >/dev/null 2>&1 || fail "$cmd 명령이 없습니다. 먼저 설치해 주세요."
done

for var in DATABASE_URL S3_BUCKET S3_ENDPOINT AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY; do
  if [ -z "${!var:-}" ]; then
...
[ -d "$UPLOAD_DIR" ] || fail "사진 폴더가 없습니다: $UPLOAD_DIR"
...
if [ "$CHECK_ONLY" = "1" ]; then
  log "명령 확인 완료 (node, mysqldump, tar, aws)"
  log "환경변수 확인 완료"
