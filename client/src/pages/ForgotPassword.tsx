...
export default function ForgotPassword() {
  ...
  // 메일을 보낼 수 없는 상태인지. 보내지도 않고 "보냈습니다" 라고 하면
  // 이용자가 오지 않을 메일을 계속 기다린다.
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      setMailUnavailable(!result.emailDelivery);
      setSent(true);
    } catch (error) {
      setMessage(
        ...
        {sent ? (
          <div className="mt-8 space-y-4 text-sm leading-7 text-[#4a4a4a]">
            {mailUnavailable ? (
              <>
                <p>
                  지금은 메일을 보내드릴 수 없습니다. 번거로우시겠지만 교회
                  사무실로 연락해 주시면 도와드리겠습니다.
                </p>
                <p className="text-[#8a8a8a]">
                  {ORG_INFO.contactPhone} · {ORG_INFO.contactEmail}
                </p>
              </>
            ) : (
              <>
