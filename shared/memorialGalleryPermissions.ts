 ...
 * 관리자가 사후에 확인한다.
 *
 * 2026-09-13: 가족 초대로 함께 관리하게 된 가족(isFamilyMember)도 주인과 같다.
 * "함께 관리하는 가족인지"는 DB 를 아는 쪽이 조회해서 넘긴다.
 */
export function canManageMemorialGallery(
  ...
  if (!canManageMemorialAsFamily(memorial, user, isFamilyMember)) return false;
  if (user?.role === "admin") return true;
  return MEMBER_EDITABLE_STATUSES.has(memorial.status);
}
