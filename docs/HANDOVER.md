...
- 안장 명단 가져오기 도구(`server/scripts/importSomangIntermentRecords.mjs`,
  ...
  보고 표를 백업한 뒤에 `--apply` 로 실제로 쓴다. 가족이 추모관을 만든 기록과
  관리자 화면에서 고친 기록(원본 자료에 `adminEditedAt` 표시)은 덮어쓰지 않는다.
- 교회의 명단 수정 요청은 **관리자 화면 → 안장 기록**(`/admin/interment`)에서
  찾아 고친다 (2026-09-23). DB 를 직접 고치지 않는다. 고치기·넣기·지우기는 관리
  기록(`interment.*`)과 한 묶음으로 남고, 추모관이 연결된 기록은 지울 수 없다.
  관리자가 넣은 기록의 원본 번호(sourceId)는 -900000001 부터 아래로 매긴다.
...
| 문의 메일 (키오스크·홈페이지) | `.env` 의 `INQUIRY_NOTIFY_EMAIL` 이 업체 메일 주소. 비어 있으면 메일은 안 가고 관리자 화면 → 운영 → 문의에만 남는다 |
...
| 추도일 알림 (카카오 알림톡) | 신청 칸은 숨겨 둠. 알리고 + 인생화원 채널로 보내기로 정함(2026-09-17). 휴면 해제·템플릿 검수·코드 작업이 남았다. 절차와 문구 초안은 [REMINDER_ALIMTALK.md](REMINDER_ALIMTALK.md) |
